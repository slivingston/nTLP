...
defined using version 1 would begin like
...
.. code-block:: xml
  ...
  <tulipcon version="1">
	...
	<anno>a string name or description; may be empty, or the <anno> tag may be omitted</anno>
    ...
    ...or... (See notes below on different types.)
    <aut type="embedded">  N.B., the root embedded automaton tag should not
                           have "trans" attribute.
      ...same as "basic" type but now with...
      <aut type="embedded" trans="i j">
        ...associated with transition from node i to node j in parent automaton.
        <aut type="embedded" trans="k l">
          ...of arbitrary depth...
        </aut>
        ...
      </aut>
...
* The types of supported automaton-based strategies are:

  basic
    ...corresponding to the most basic finite automaton, as found in
    previous versions of TuLiP.

  embedded
    There is a root automaton with transitions that may correspond to
    automata representing "reachability game" strategies. More
    tutorial-like documentation on these is forthcoming. N.B., the
    implementation of "embedded automata" is not yet public.
...
* An important use case of the <anno> element (under <aut>) is
  experimenting with node annotation, which may later receive
  dedicated elements in this XML schema.
...
missing or incomplete by using default values. Pseudo-code describing
the specification follows. It is *not normative*.
...
A partial listing of related scripts follows. Please see internal
documentation of these for details.

``tools/autopart.py``
  Read YAML file, perform refinement based on
  dynamics ("discretization"), and save result to tulipcon XML file.

``tools/gengrid.py``
  Create a grid partition in the plane.
